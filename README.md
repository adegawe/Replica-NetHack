...
맵을 팔분원(octant) 8개로 나누고, 각 팔분원마다 거리 단위로 스캔하면서 벽을 만나면 시야 범위를 분할해 재귀 호출하는 방식으로 구현했다. 타일이 점이 아니라 부피를 가진 단위로 판정되도록 `leftSlope`/`rightSlope`에 0.8f 오프셋을 적용해, 대각선 경계에서 타일이 간헐적으로 빠지는 문제를 해결했다.

...
이를 해결하기 위해 BSP(Binary Space Partitioning) 방식으로 교체할 예정이다. BSP는 전체 맵 공간을 재귀적으로 이분할한 뒤, 각 리프 노드에 방을 배치하는 방식이다. 이 방식을 선택한 이유는 두 가지다.
...
<!-- 
  여기에 BSP 과정을 보여주는 간단한 다이어그램이나 
  before/after 스크린샷을 넣으면 효과적.
-->
...
- **BSP 던전 생성** — 현재 랜덤 배치 방식을 BSP로 교체
